...
const TestimonialCard = ({ testimonial, className = "" }: TestimonialCardProps) => {
  ...
  // Determine if truncation is needed - increased character count for better display
  const needsTruncation = testimonial.translation.length > 100 || testimonial.quote.length > 120;
  
  // Always show meaningful content regardless of language
  const truncatedQuote = needsTruncation && !expanded
    ? `${testimonial.quote.substring(0, 120)}${testimonial.quote.length > 120 ? '...' : ''}`
    : testimonial.quote;
    
  const truncatedTranslation = needsTruncation && !expanded
    ? `${testimonial.translation.substring(0, 100)}${testimonial.translation.length > 100 ? '...' : ''}`
    : testimonial.translation;
  
  return (
    <div 
      className={`bg-white p-4 sm:p-5 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 border border-red-100 flex flex-col h-full ${className}`}
    >
      {/* Card header with quote icon and rating */}
      <div className="flex justify-between items-start mb-3">
        <Quote className="h-6 w-6 text-red-400 flex-shrink-0" />
        <div className="flex">
          {[...Array(5)].map((_, i) => (
            <svg 
              key={i}
      ...
      {/* Content area with proper Tamil and English text display */}
      <div className="flex-grow flex flex-col">
        {/* Tamil quote with enhanced visibility */}
        <div className="mb-2">
          <p className="text-gray-800 text-sm italic font-medium" dir="auto">
            {expanded ? testimonial.quote : truncatedQuote}
          </p>
        </div>
        
        {/* English translation with better display */}
        <div className="text-gray-600 text-sm mb-3" dir="auto">
          <p>{expanded ? testimonial.translation : truncatedTranslation}</p>
        </div>
        
        {/* Read more button with better positioning */}
        {needsTruncation && (
          <button
            onClick={() => setExpanded(!expanded)}
            className="text-red-500 hover:text-red-700 text-xs font-medium flex items-center mt-auto mb-3 transition-colors"
      ...
      {/* Footer with guaranteed visibility and proper spacing */}
      <div className="flex items-center pt-3 border-t border-gray-100 mt-auto">
        <div className="relative h-10 w-10 rounded-full overflow-hidden border-2 border-red-200 flex-shrink-0">
          <AvatarImage 
...
const TestimonialsSection = () => {
  ...
  return (
    <section ref={containerRef} className="py-10 sm:py-12 lg:py-16 bg-gradient-to-b from-white to-red-50 overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative mb-8">
          ...
          >
            {/* Using CSS grid with fixed widths on mobile to prevent alignment issues */}
            <motion.div 
              className="flex gap-4 sm:gap-5 py-2 w-full"
              initial={{ x: 0 }}
              animate={{ 
              ...
              style={{ 
                // Ensure consistent height with enough space for expanded content
                minHeight: screenSize === 'xs' || screenSize === 'sm' ? '420px' : '380px'
              }}
            >
              {testimonials.map((testimonial) => (
                <motion.div 
                  ...
                  className={`flex-shrink-0 ${
                    ...
                    screenSize === 'md' ? 'w-[calc(50%-8px)]' : 
                    'w-[calc(33.333%-10.67px)]'
                  }`}
                  initial={{ opacity: 0, y: 20 }}
                  animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
